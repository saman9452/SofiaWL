...
namespace RelationalGit.Calculation
{
    public class Analyzer
    {
        public void AnalyzeSimulations(long actualSimulationId, long[] recommenderSimulationIds, string analyzeResultPath)
        {
            ...
            CalculateLearnerReviews(actualSimulationId, recommenderSimulationIds, analyzeResultPath);
            CalculateFaRReduction(actualSimulationId, recommenderSimulationIds, analyzeResultPath);
            CalculateNewFar(actualSimulationId, recommenderSimulationIds, analyzeResultPath);
        ...
        public void CalculateFaRReduction_(long actualId, long[] simulationsIds, string path)
        {
            if (!Directory.Exists(path))
                Directory.CreateDirectory(path);
            ...
            using (var dbContext = GetDbContext())
            {
                ...
                foreach (var simulationId in simulationsIds)
                {
                    var affectedSimulation = dbContext.AffectedLeaverFiles.Where(a => a.SimulationId == simulationId).Select(a => a.FilePath).ToList();
                    var actualFaR = dbContext.FileKnowledgeables.Where(q => affectedSimulation.Contains(q.CanonicalPath) && q.HasReviewed && q.TotalKnowledgeables < 2 && q.LossSimulationId == actualId).
                    GroupBy(q => q.PeriodId).
                    Select(q => new { Count = q.Count(), PeriodId = q.Key })
                    .ToArray();
                    ...
                    var lossSimulation = dbContext.LossSimulations.Single(q => q.Id == simulationId);
                    var test = dbContext.FileKnowledgeables.Where(q => affectedSimulation.Contains(q.CanonicalPath) && q.HasReviewed && q.TotalKnowledgeables < 2 && q.LossSimulationId == simulationId).ToArray();


                    var simulatedFaR = dbContext.FileKnowledgeables.Where(q => affectedSimulation.Contains(q.CanonicalPath) && q.HasReviewed && q.TotalKnowledgeables < 2 && q.LossSimulationId == simulationId).
                        GroupBy(q => q.PeriodId).
                        Select(q => new { Count = q.Count(), PeriodId = q.Key })
                        .ToArray();
            ...
            Write(result, Path.Combine(path, "Affectedfar.csv"));

        }
        public void CalculateFaRReduction__(long actualId, long[] simulationsIds, string path)
        {
            if (!Directory.Exists(path))
                Directory.CreateDirectory(path);
            ...
            using (var dbContext = GetDbContext())
            {
                ...
                Dictionary<string, long> leaveTime = new Dictionary<string, long>();
                var developers = dbContext.Developers.ToList();
                for (int i = 0; i < developers.Count; i++)
                {
                    var lastcontribution = Math.Max(developers[i].LastReviewPeriodId ?? 1, developers[i].LastCommitPeriodId ?? 1);
                    leaveTime.Add(developers[i].NormalizedName, lastcontribution);

                }
                var farperdeveloper = new List<double>();
                foreach (var simulationId in simulationsIds)
                {
                    var count = leaveTime.Count;
                    foreach (var item in leaveTime)
                    {
                         count--;
                        var files = new List<string>();
                        var simulated_files = new List<string>();
                        var t1 = item.Key + ",";
                        var t2 = "," + item.Key + ",";
                       
                       
                        var knowledgeables = dbContext.FileKnowledgeables.Where(a => a.LossSimulationId == actualId && a.PeriodId < item.Value && a.TotalKnowledgeables == 2 && (a.Knowledgeables.Contains(t1) || a.Knowledgeables.Contains(t2) || a.Knowledgeables.EndsWith(item.Key))).Select(a => a.CanonicalPath);
                        files.AddRange(knowledgeables);
                        var simulated_knowledgeables = dbContext.FileKnowledgeables.Where(a => a.LossSimulationId == simulationId && a.PeriodId < item.Value && a.TotalKnowledgeables == 2 && (a.Knowledgeables.Contains(t1) || a.Knowledgeables.Contains(t2) || a.Knowledgeables.EndsWith(item.Key))).Select(a => a.CanonicalPath);
                        simulated_files.AddRange(simulated_knowledgeables);
                        var actualFaR = dbContext.FileKnowledgeables.Where(a =>a.LossSimulationId == actualId && a.PeriodId > item.Value && files.Contains(a.CanonicalPath) && a.TotalKnowledgeables<2).GroupBy(a => a.PeriodId).Select(q => new { Count = q.Count(), PeriodId = q.Key })
                        .ToArray();
                        if (simulated_files.Count==0 || actualFaR.Count() == 0)
                        {
                            continue;
                        }
                        var lastPeriod = actualFaR.Max(q => q.PeriodId);
                        actualFaR = actualFaR.Where(q => q.PeriodId != lastPeriod)
                            .ToArray();
                        var simulatedFaR = dbContext.FileKnowledgeables.Where(a => a.LossSimulationId == simulationId && a.PeriodId > item.Value && simulated_files.Contains(a.CanonicalPath)&& a.TotalKnowledgeables<2).GroupBy(a => a.PeriodId).Select(q => new { Count = q.Count(), PeriodId = q.Key })
                            .ToArray();
                        var lossSimulation = dbContext.LossSimulations.Single(q => q.Id == simulationId);
                        var simulationResult = new SimulationResult()
                        {
                        ...
                        if(simulationResult.Results != null && simulationResult.Results.Count!=0)
                            farperdeveloper.Add(simulationResult.Average);
                    }


                    var final = farperdeveloper.Average();
                }
              
            }

            Write(result, Path.Combine(path, "NewFar.csv"));

        }
        public void CalculateNewFar(long actualId, long[] simulationsIds, string path)
        {
            if (!Directory.Exists(path))
                Directory.CreateDirectory(path);
            ...
            using (var dbContext = GetDbContext())
            {
                var actualFaR = dbContext.FileKnowledgeables.Where(q => q.HasReviewed && q.TotalKnowledgeables < 2 && q.LossSimulationId == actualId).
                    GroupBy(q => q.PeriodId).
                    Select(q => new { Count = q.Count(), PeriodId = q.Key, files =q.Select(a => a.CanonicalPath) })
                    .ToArray();
                ...
                foreach (var simulationId in simulationsIds)
                {
                    ...
                    foreach (var simulatedFaRPeriod in simulatedFaR)
                    {
                        ...
                        if (actualValue == null)
                            continue;
                        if (simulatedFaRPeriod.PeriodId != 1)
                        {
                            var actualValuepre = actualFaR.SingleOrDefault(q => q.PeriodId == simulatedFaRPeriod.PeriodId - 1);
                            var actual_diff = 0;
                            if (actualValuepre == null)
                                actual_diff = actualValue.Count - 0;
                            else
                            {
                                
                                foreach(var item in actualValue.files)
                                {
                                    if (actualValuepre.files.Contains(item))
                                    {
                                        continue;
                                    }
                                    else
                                ...
                                /*foreach (var item in actualValuepre.files)
                                {
                                    if (actualValue.files.Contains(item))
                                    {
                                        continue;
                                    }
                                    else
                            ...
                            var simulatedpre = simulatedFaR.SingleOrDefault(q => q.PeriodId == simulatedFaRPeriod.PeriodId - 1);
                            var simulated_diff = 0;
                            if (simulatedpre == null)
                                simulated_diff = simulatedFaRPeriod.Count - 0;
                            else
                            {
                                foreach (var item in simulatedFaRPeriod.files)
                                {
                                    if (simulatedpre.files.Contains(item))
                                    {
                                        continue;
                                    }
                                    else
                                ...
                                /*foreach (var item in simulatedpre.files)
                                {
                                    if (simulatedFaRPeriod.files.Contains(item))
                                    {
                                        continue;
                                    }
                                    else
                            ...
                            var value = CalculateIncreasePercentage(simulated_diff, actual_diff);
                            simulationResult.Results.Add((simulatedFaRPeriod.PeriodId, value));
                        }
  ...
  order by pulls desc";
        ...
        private static void CalculateLearnerReviews(long actualId, long[] simulationsIds, string path)
        {
            var result = new List<OpenReviewResult>();
            foreach (var simulationId in simulationsIds)
            {
     ...
     count(distinct(pullRequestId)) as pulls,
      DATEPART(QUARTER, DateTime) ,  DATEPART(year, Datetime)
  FROM [dbo].[LearnerReviewers] where SimulationId = @simId  Group by DATEPART(QUARTER, Datetime)  , NormalizedName,  DATEPART(year, Datetime)
  order by pulls desc";
                    SqlCommand command = new SqlCommand(queryString, connection);
                    command.Parameters.AddWithValue("@simId", simulationId);
                    connection.Open();
            ...
            WriteOpenReviws(result, Path.Combine(path, "auc2.csv"));
        }
